...
- **`movementStartDistance`** / **`movementFarDistance`** — *optional* exact distances (in blocks) that replace the two derived movement triggers. They control when the wizard casts its **movement spell** (teleport/dash, e.g. `blood_step`) to jump closer to a target that is too far. The difference:
  - **`movementFarDistance`** is the *hard* threshold. When the target is **farther than this**, the wizard strongly wants the movement spell (the desire jumps high, scaling up the farther the target is) — "the target is definitely out of comfortable range, close the gap now".
  - **`movementStartDistance`** is the *soft* threshold. When the target is **between** `movementStartDistance` and `movementFarDistance`, the desire **ramps up gradually** (from none at `start` to strong at `far`) — "the target is getting far, starting to consider repositioning".
  - A target closer than `movementStartDistance` produces no distance-driven movement desire (the wizard attacks/strafes instead).
  - When both are `0` (default), they are derived from the spell range: `movementStartDistance = 0.75 × (range − movementDistanceOffset)` and `movementFarDistance = (range − movementDistanceOffset)`.
  - Worked example (spells reach 20 blocks, `movementDistanceOffset: 5` → derived `start ≈ 11`, `far = 15`): target at 8 blocks → no movement desire; at 13 blocks → ramping desire; at 16 blocks → strong desire, the wizard casts `blood_step` to jump closer.
  - Set them explicitly to take full control (e.g. `"movementStartDistance": 15.0, "movementFarDistance": 20.0`).
- **`movementTooCloseDistance`** — how close (in blocks) the target must be before the wizard uses its movement spell to **back away / reposition** instead of standing point-blank. Default `5.0` (also the close-wizard standoff). Set `0` to disable the too-close repositioning.
