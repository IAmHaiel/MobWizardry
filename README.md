...
- **`movementStartDistance`** / **`movementFarDistance`** — *optional* exact distances (in blocks) that replace the derived far/start movement triggers. `movementFarDistance` is the distance at which the wizard strongly wants to use a movement spell to close the gap; `movementStartDistance` is where the desire starts ramping up. When both are `0` (default), they are derived from the spell range (`0.75 × range` and `range`) minus `movementDistanceOffset`. Set them to take full control (e.g. `"movementStartDistance": 15.0, "movementFarDistance": 20.0`).
